...
class Lights(commands.Cog, name="Lighting Commands"):
    ...
    async def on(self, ctx):
        ...
        req = requests.put(
            f"http://{self.bot.config.aurora_ip}:16021/api/v1/{self.bot.config.aurora_auth}/state",
            json=data,
            headers=headers,
        )
        ...
        if req.status_code == 204:
            ...
            await Utils().sendEmbed(
                ctx, description=f"Could not complete operation.",
            )
    ...
    async def brightness(self, ctx, *args):
        ...
        if args:
            ...
            req = requests.get(
                f"http://{self.bot.config.aurora_ip}:16021/api/v1/{self.bot.config.aurora_auth}/state/brightness"
            )
    ...
    async def hue(self, ctx, *args):
        ...
        if args:
            ...
            req = requests.get(
                f"http://{self.bot.config.aurora_ip}:16021/api/v1/{self.bot.config.aurora_auth}/state/hue"
            )
    ...
    async def saturation(self, ctx, *args):
        ...
        if args:
            ...
            req = requests.get(
                f"http://{self.bot.config.aurora_ip}:16021/api/v1/{self.bot.config.aurora_auth}/state/sat"
            )
    ...
    async def listeffects(self, ctx):
        ...
        """
        req = requests.get(
            f"http://{self.bot.config.aurora_ip}:16021/api/v1/{self.bot.config.aurora_auth}/effects/effectsList"
        )
        ...
        if req.status_code == 200:
            ...
            await Utils().sendEmbed(
                ctx,
                title=f"Available Effects on {self.bot.deviceName}",
    ...
    async def seteffect(self, ctx, *, args):
        ...
        req = requests.put(
            f"http://{self.bot.config.aurora_ip}:16021/api/v1/{self.bot.config.aurora_auth}/effects",
            json=data,
            headers=headers,
        )
        ...
        if req.status_code == 204:
            await Utils().sendEmbed(
                ctx, description=f":milky_way: Successfully set effect to `{args}`."
            )
        else:
            await ctx.send(
                f":x: Couldn't find an effect by that name. Check your spelling and try again."
            )
